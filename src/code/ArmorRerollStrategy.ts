...
class ArmorRerollStrategy implements RS.RerollStrategy {
  pickRerollDice(
    ...
    if (input.offense.ramX.active && input.combat.meleeAttack) {
      ...
      ramRemaining -= dmm.tryConvertResultCount(
        T.AttackDieResult.Miss,
        T.AttackDieResult.Critical,
        ramRemaining
      );
    ...
    const possibleHits = Math.min(
      ...
      totalHits + RSH.getMaximumRerolledDice(input, remaining)
    );
    let hitsToKeep = 0;
    ...
    if (reason === RS.RerollReason.AimToken) {
      ...
      const impactConvert = Math.max(
        0,
        dmm.getResultCount(T.AttackDieResult.Hit) - coverModifier
      );
      dmm.tryConvertResultCount(
        T.AttackDieResult.Hit,
        T.AttackDieResult.Critical,
