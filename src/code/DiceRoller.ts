...
export class DiceRoller {
  ...
  public simulateAttacks(
    ...
    let maxCount = Math.max(
      attackSummary.critical.length,
      attackSummary.hit.length
    );
    ...
    for (let i = maxCount - 1; i >= 0; i--) {
      if (
        attackSummary.critical[i] === undefined &&
        attackSummary.critical[i + 1] !== undefined
      ) {
        attackSummary.critical[i] = 0;
      }
      if (
        attackSummary.hit[i] === undefined &&
        attackSummary.hit[i + 1] !== undefined
      ) {
        attackSummary.hit[i] = 0;
      }
      if (
        attackSummary.attackSurge[i] === undefined &&
        attackSummary.attackSurge[i + 1] !== undefined
      ) {
        attackSummary.attackSurge[i] = 0;
      }
      if (
        attackSummary.forcedSaves[i] === undefined &&
      ...
      if (
        ...
        attackSummary.blocks[i + 1] !== undefined
      ) {
        attackSummary.blocks[i] = 0;
      }
      if (
        attackSummary.defenseSurge[i] === undefined &&
        attackSummary.defenseSurge[i + 1] !== undefined
      ) {
        attackSummary.defenseSurge[i] = 0;
      }
      if (
        attackSummary.wounds[i] === undefined &&
        attackSummary.wounds[i + 1] !== undefined
      ) {
        attackSummary.wounds[i] = 0;
      }
    }
  ...
  private defaultRollAttackDie(color: T.DieColor): T.AttackDieResult {
    ...
    if (
      (number === 0 && color === T.DieColor.White) ||
  ...
  private rollAttackDice(
    ...
    // makashi mastery
    if (
      modifiedInput.offense.makashiMastery &&
      modifiedInput.combat.meleeAttack &&
      modifiedInput.offense.pierceX.active &&
      modifiedInput.offense.pierceX.value > 1 &&
      (modifiedInput.defense.immunePierce ||
        modifiedInput.defense.immuneMeleePierce ||
        modifiedInput.defense.impervious)
    ) {
    ...
    if (modifiedInput.offense.duelist && modifiedInput.combat.meleeAttack) {
      ...
      if (
        modifiedInput.offense.tokens.aim === input.offense.tokens.aim &&
        input.offense.tokens.aim > 0
      ) {
  ...
  private rerollForObservationTokens(
    modifiedInput: T.AttackInput,
    rolls: T.AttackRoll[],
    ...
    for (let i = 0; i < modifiedInput.defense.tokens.observation; i++) {
      if (
        !this.rerollAttackDiceWithStrategy(
          modifiedInput,
          rolls,
          1,
          RS.RerollReason.ObservationToken,
          remaining
        )
      ) {
        break;
  ...
  private rerollAttackDiceWithStrategy(
    modifiedInput: T.AttackInput,
    rolls: T.AttackRoll[],
    count: number,
    ...
    const indexes = rerollStrategy.pickRerollDice(
      modifiedInput,
      rolls,
      count,
      reason,
  ...
  private convertAttackSurges(
    ...
    if (modifiedInput.offense.criticalX.active) {
      status.modificationMatrix.tryConvertResultCount(
        T.AttackDieResult.Surge,
        T.AttackDieResult.Critical,
        modifiedInput.offense.criticalX.value
      );
    }

    // surge -> crit
    let surgeToCrit =
      modifiedInput.offense.surge === T.AttackSurgeConversion.Critical;

    // jedi hunter
    surgeToCrit =
      surgeToCrit ||
      (modifiedInput.offense.jediHunter &&
        modifiedInput.defense.hasForceUpgrades);
    ...
    if (
      !surgeToCrit &&
      modifiedInput.offense.surge === T.AttackSurgeConversion.Hit
    ) {
      status.modificationMatrix.tryConvertResultAllExcept(
        T.AttackDieResult.Surge,
    ...
    if (modifiedInput.offense.tokens.surge > 0) {
      modifiedInput.offense.surge -=
        status.modificationMatrix.tryConvertResultCount(
          T.AttackDieResult.Surge,
          T.AttackDieResult.Hit,
          modifiedInput.offense.tokens.surge
        );
    }
  ...
  private applyDodgeAndCover(
    ...
    if (
      !modifiedInput.offense.highVelocity &&
      ...
      // block and deflect
      if (
        (modifiedInput.defense.block ||
          modifiedInput.defense.deflect ||
          modifiedInput.defense.soresuMastery ||
          modifiedInput.defense.djemSoMastery) &&
    ...
    if (modifiedInput.offense.ionX.active) {
      const ionXValue = Math.min(
        modifiedInput.offense.ionX.value,
        status.modificationMatrix.getResultCount(T.AttackDieResult.Critical) +
          status.modificationMatrix.getResultCount(T.AttackDieResult.Hit)
      );
      modifiedInput.defense.tokens.shield -= Math.min(
        ionXValue,
  ...
  private modifyAttackDice(
    ...
    if (modifiedInput.defense.armor || modifiedInput.defense.armorX.active) {
      if (modifiedInput.offense.impactX.active) {
        status.modificationMatrix.tryConvertResultCount(
          T.AttackDieResult.Hit,
          T.AttackDieResult.Critical,
          modifiedInput.offense.impactX.value
        );
      }
    }
    ...
    if (modifiedInput.defense.armorX.active) {
      status.modificationMatrix.tryConvertResultCount(
        T.AttackDieResult.Hit,
        T.AttackDieResult.Miss,
        modifiedInput.defense.armorX.value
      );
    }
  ...
  private rollDefenseDice(
    ...
    // danger sense
    if (
      modifiedInput.defense.dangerSenseX.active &&
      modifiedInput.defense.tokens.suppression > 0
    ) {
  ...
  private rerollDefenseDice(
    ...
    if (modifiedInput.defense.uncannyLuckX.active) {
      ...
      const rerolls = Math.min(
        maxRerolls,
        modifiedInput.defense.uncannyLuckX.value
      );
  ...
  private convertDefenseSurges(
    ...
    // surge -> block
    if (modifiedInput.defense.surge) {
      status.modifiedDefenseResult.blocks +=
        status.modifiedDefenseResult.surges;
      status.modifiedDefenseResult.surges = 0;
    }

    // surge tokens
    const spentSurgeTokens = Math.min(
      modifiedInput.defense.tokens.surge,
      status.modifiedDefenseResult.surges
    );
  ...
  private modifyDefenseDice(
    ...
    // apply pierce
    if (modifiedInput.offense.pierceX.active) {
      const effectivePierce = Math.min(
        status.modifiedDefenseResult.blocks,
        modifiedInput.offense.pierceX.value
      );
  ...
  private compareResults(status: DRT.AttackResultStatus) {
    status.defenseResult.wounds =
      status.defenseResult.forcedSaves - status.modifiedDefenseResult.blocks;
  }
}
