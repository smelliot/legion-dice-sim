...
class ArmorXRerollStrategy implements RS.RerollStrategy {
  pickRerollDice(
    ...
    const possibleHits = Math.min(
      ...
      startingHits + RSH.getMaximumRerolledDice(input, remaining)
    );
    ...
    if (reason === RS.RerollReason.AimToken) {
      ...
      const conversions = RSH.getMarksmanConversions(
        input,
        hitsForMarksman,
        critsForMarksman,
        remaining
    ...
    // convert for armor x
    const remainingArmor = Math.max(
      0,
      input.defense.armorX.value - hitsAfterRam
    );
    ...
    if (reason === RS.RerollReason.AimToken) {
      const effectiveLethal = RSH.getEffectiveLethal(
        input,
        hitsAfterArmor,
        totalCrits + effectiveImpact
      );
    ...
    if (lethalOverride) {
      ...
      possibleHits <= coverModifier + input.defense.armorX.value
    ) {
      rerollHit = rerollDiceCount - rerollMiss - effectiveRam;
    } else if (
      input.offense.impactX.active &&
      possibleHits <= coverModifier + input.offense.impactX.value
    ) {
      rerollHit = 0;
    } else if (
      input.offense.impactX.active &&
      possibleHits <=
        coverModifier + input.offense.impactX.value + input.defense.armorX.value
    ) {
      rerollHit =
        possibleHits -
        (coverModifier + input.offense.impactX.value) -
        effectiveRam;
    } else {
      // enough dice to get past the armor and hits to count as damage
      rerollHit = 0;
    }
  ...
  public shouldHandle(input: T.AttackInput, _reason: RS.RerollReason): boolean {
