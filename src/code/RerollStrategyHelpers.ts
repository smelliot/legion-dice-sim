...
export function getEffectiveLethal(
  input: T.AttackInput,
  effectiveHits: number,
  criticals: number
): number {
  const immunePierce =
    input.defense.immunePierce ||
    ((input.defense.duelist || input.defense.immuneMeleePierce) &&
  ...
  if (input.offense.lethalX.active && !immunePierce) {
    ...
    effectiveDamage = Math.max(
      0,
      effectiveDamage - input.defense.tokens.shield
    );
    let pierceValue = input.offense.pierceX.active
      ? input.offense.pierceX.value
      : 0;
...
export function getMarksmanConversions(
  input: T.AttackInput,
  hits: number,
  crits: number,
  rerolls: RS.RemainingRerolls
): Conversions {
  const conversions = {
    hitsToCrits: 0,
    blanksToHits: 0,
  ...
  while (aimTokens > effectiveLethal) {
    ...
    let impact = 0;
    if (
      input.offense.impactX.active &&
      (input.defense.armor || input.defense.armorX.active)
    ) {
    ...
    if (input.offense.ramX.active) {
      const missChange = Math.min(input.offense.ramX.value, workingMiss);
      workingCrits += missChange;

      const hitChange = Math.min(
        input.offense.ramX.value - missChange,
        workingHits
      );
      workingHits -= hitChange;
      workingCrits += hitChange;
    }

    effectiveLethal = getEffectiveLethal(input, workingHits, workingCrits);

    const availableAims = aimTokens - effectiveLethal;
    const convertHits = Math.max(
      0,
      Math.min(
        availableAims,
        totalDice - (conversions.blanksToHits + conversions.hitsToCrits)
      )
    );
    const pre = {
    ...
    if (availableAims > 0) {
      if (input.defense.armor) {
        ...
        if (
          totalDice -
            workingCrits -
            Math.max(workingHits - input.defense.armorX.value, 0) ===
            maxCancelledHits &&
        ...
        if (
          lostHits === maxCancelledHits &&
          (remainingImpact > 0 || workingHits >= input.defense.armorX.value) &&
          startingMiss > 0
        ) {
          conversions.blanksToHits++;
          aimTokens--;
          ...
          aimTokens >= 2 &&
          totalDice - (hits + crits) - conversions.hitsToCrits > 0
        ) {
          conversions.blanksToHits++;
          conversions.hitsToCrits++;
          aimTokens -= 2;
        ...
        if (
          maxCancelledHits > 0 &&
          aimTokens >= 2 &&
          convertHits <= maxCancelledHits + 1
        ) {
          conversions.blanksToHits++;
          conversions.hitsToCrits++;
          aimTokens -= 2;
    ...
    conversions.blanksToHits = Math.min(
      conversions.blanksToHits,
      totalDice - (hits + crits)
    );
    conversions.hitsToCrits = Math.min(
      conversions.hitsToCrits,
      totalDice - crits
    );

    if (
      pre.blanksToHits === conversions.blanksToHits &&
      pre.hitsToCrits === conversions.hitsToCrits
    ) {
      break;
    }
  }
...
export function getMaximumRerolledDice(
  input: T.AttackInput,
  rerolls: RS.RemainingRerolls
): number {
  return (
    rerolls.observationTokens +
    rerolls.aimTokens *
      (2 + (input.offense.preciseX.active ? input.offense.preciseX.value : 0))
  );
}
