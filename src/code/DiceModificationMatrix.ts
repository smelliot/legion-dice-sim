...
class DiceModificationMatrix {
  ...
  public getRerollIndexes(hits: number, misses: number): number[] {
    ...
    const matchList = this.getConversions().filter(
      (element: DieConversion): boolean => {
        if (
          element.conversion === T.AttackDieResult.Miss &&
          remainingMiss > 0
        ) {
          remainingMiss--;
          return true;
    ...
    matchList.forEach((element: DieConversion) => {
      ...
      for (let i = 0; i < criteria.length; i++) {
        if (
          criteria[i].roll.color === element.roll.color &&
    ...
    this._rolls.forEach((element: T.AttackRoll, idx: number) => {
      for (let i = 0; i < criteria.length; i++) {
        if (
          criteria[i].roll.color === element.color &&
  ...
  private addConversions(color: T.DieColor, conversions: DieConversion[]) {
    ...
    for (let row = 0; row < 4; row++) {
      for (let col = 0; col < 4; col++) {
        const count =
          row === col
            ? 2 * matrix[row][col] - this.sumRow(row, matrix)
            : matrix[row][col];
  ...
  public tryConvertResult(
    originalResult: T.AttackDieResult,
    newResult: T.AttackDieResult
  ): boolean {
    if (this.isConvertingUp(originalResult, newResult)) {
      return (
        this.tryConvertResultForColor(
          T.DieColor.White,
          originalResult,
          newResult
        ) ||
  ...
  public tryConvertResultCount(
    oldResult: T.AttackDieResult,
    newResult: T.AttackDieResult,
    count: number
  ): number {
    let converted = 0;
  ...
  public tryConvertResultAllExcept(
    oldResult: T.AttackDieResult,
    newResult: T.AttackDieResult,
    maintain: number
  ): number {
  ...
  private getResultCountInMatrix(
    matrix: number[][],
    result: T.AttackDieResult
  ): number {
    const index = this.idx4ar(result);
    return (
      matrix[index][index] +
      this.sumColumn(index, matrix) -
      this.sumRow(index, matrix)
    );
  }

  private tryConvertResultForColor(
    color: T.DieColor,
    originalResult: T.AttackDieResult,
    newResult: T.AttackDieResult
  ): boolean {
  ...
  private idx4ar(result: T.AttackDieResult): number {
    return result === T.AttackDieResult.Critical
      ? iCrit
      : result === T.AttackDieResult.Hit
        ? iHit
        : result === T.AttackDieResult.Surge
          ? iSurge
          : iMiss;
  }
  ...
  private isConvertingUp(
    originalResult: T.AttackDieResult,
    newResult: T.AttackDieResult
  ): boolean {
    return (
      newResult === T.AttackDieResult.Critical ||
      originalResult === T.AttackDieResult.Miss ||
      (originalResult === T.AttackDieResult.Surge &&
        newResult !== T.AttackDieResult.Miss)
    );
  }
}
