...
function isPositionCompatible(
  weapon: UP.Weapon,
  position: Array<UP.FixedPosition> | null
): boolean {
...
function getPositionOverlap(
  weapon: UP.Weapon,
  position: Array<UP.FixedPosition> | null
): Array<UP.FixedPosition> | null {
  if (weapon.keywords?.fixed && position) {
...
function determineSidearmAction(
  upgrade: UC.WeaponUpgrade,
  tracking: Tracking
): SidearmAction {
    ...
    switch (upgrade.weapon.keywords?.sidearm) {
      case UP.Sidearm.ranged:
        if (tracking.maximumRange === undefined || tracking.maximumRange > 0) {
          if (
            !UP.isRangeCompatible(
              upgrade.weapon,
              tracking.minimumRange,
              tracking.maximumRange
            )
          ) {
...
function applyUpgradeKeywords(upgrade: UC.Upgrade, tracking: Tracking) {
  if (upgrade.keywords) {
    ...
    if (
      upgrade.keywords.surgeHit &&
      tracking.offense.surge === T.AttackSurgeConversion.Blank
    ) {
      tracking.offense.surge = T.AttackSurgeConversion.Hit;
    }
    applyValueXKeyword(upgrade.keywords.precise, tracking.offense.preciseX);
    applyValueXKeyword(
      upgrade.keywords.sharpshooter,
      tracking.offense.sharpshooterX
    );
  }
}

function applyWeaponKeywords(
...
function applyWeapon(
  ...
  if (
    !UP.isRangeCompatible(weapon, tracking.minimumRange, tracking.maximumRange)
  ) {
    return;
  }
  ...
  if (tracking.maximumRange === undefined) {
    ...
    tracking.maximumRange = Math.min(
      weapon.maximumRange,
      tracking.maximumRange
    );
  }
...
function applyHeavyWeaponUpgrade(
  upgrade: UC.HeavyWeaponUpgrade,
  tracking: Tracking
) {
  if (upgrade.weapon) {
...
function applyPersonnelUpgrade(
  upgrade: UC.PersonnelUpgradeCard,
  tracking: Tracking
) {
...
function applyUpgrade(tracking: Tracking, upgrade: UC.Upgrade): void {
  switch (upgrade.type) {
    ...
    case UP.UnitUpgrade.counterpart:
      applyWeaponUpgrade(
        upgrade as UC.WeaponUpgrade,
        (upgrade as UC.CounterpartUpgrade).miniCount,
        tracking,
        false
      );
      break;
...
function applySpecialUpgrades(upgrades: Array<UC.Upgrade>, tracking: Tracking) {
  if (
    upgrades.filter(
      (u) => u.type === UP.UnitUpgrade.force && u.name === "Saber Throw"
    ).length > 0
  ) {
    if (tracking.activeWeapons === 1 && tracking.maximumRange === 0) {
      const totalDice =
        tracking.offense.redDice +
        tracking.offense.blackDice +
        tracking.offense.whiteDice;
      const halfDice = Math.floor((totalDice + 1) / 2);
      tracking.offense.redDice = Math.min(tracking.offense.redDice, halfDice);
      tracking.offense.blackDice = Math.min(
        tracking.offense.blackDice,
        halfDice - tracking.offense.redDice
      );
      tracking.offense.whiteDice = Math.min(
        tracking.offense.whiteDice,
        halfDice - tracking.offense.redDice - tracking.offense.blackDice
      );
...
function createTrackingObject(
  ...
  upgrades.forEach((u) => {
    ...
    if (
      u.type === UP.UnitUpgrade.armament ||
  ...
  return {
    ...
    activeWeapons: 0,
    defaultWeapon: null,
    useUnitWeaponCount: Math.min(
      Math.max(0, arsenal - (totalWeapons - weapons.length)),
      weapons.length
    ),
    minimumRange: 0,
    fixedPosition: null,
    ...
    offense: {
      ...
      preciseX: profile.keywords?.precise
        ? { active: true, value: profile.keywords.precise }
...
export function createAttackInputsFromProfile(
  profile: UP.UnitProfile,
  weapons: Array<UP.Weapon>,
  upgrades: Array<UC.Upgrade>,
  tokens: T.OffenseTokens
  ...
  weapons.forEach((w) => {
    if (
      usedUnitWeapons < tracking.useUnitWeaponCount &&
  ...
  input.combat.meleeAttack =
    tracking?.maximumRange !== undefined ? tracking?.maximumRange === 0 : false;
