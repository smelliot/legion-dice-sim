...
const units: Array<UnitProfile> = (<Array<UnitProfile>>unitsFile.units).sort(
  (first: UnitProfile, second: UnitProfile) => {
...
export function isRangeCompatible(
  weapon: Weapon,
  minimumRange: number,
  maximumRange?: number
): boolean {
  ...
  if (maximumRange === undefined) {
    return (
      weapon.maximumRange !== undefined && weapon.maximumRange >= minimumRange
    );
  }
  ...
  return (
    (weapon.maximumRange >= minimumRange &&
      weapon.maximumRange <= maximumRange) ||
    (weapon.minimumRange >= minimumRange &&
      weapon.minimumRange <= maximumRange) ||
    (maximumRange >= weapon.minimumRange &&
...
export function convertUnitProfileSurgeToAttackSurge(
  surge: AttackSurge
): T.AttackSurgeConversion {
  switch (surge) {
