...
function applyUpgrade(upgrade: UC.Upgrade, tracking: Tracking) {
  if (upgrade.keywords) {
    ...
    if (upgrade.keywords.cover === T.Cover.Heavy) {
      ...
      tracking.defense.cover =
        tracking.defense.cover === T.Cover.None ? T.Cover.Light : T.Cover.Heavy;
    }

    applyValueXKeyword(
      upgrade.keywords.dangerSense,
      tracking.defense.dangerSenseX
    );
    ...
    applyValueXKeyword(
      upgrade.keywords.uncannyLuck,
      tracking.defense.uncannyLuckX
    );
  }
}
...
function applySpecialUpgrades(upgrades: Array<UC.Upgrade>, tracking: Tracking) {
  if (
    upgrades.filter(
      (u) => u.type === UP.UnitUpgrade.training && u.name === "Duck and Cover"
    ).length > 0
  ) {
...
function createTrackingObject(
  ...
  return {
    defense: {
      ...
      hasForceUpgrades:
        (profile.upgrades || []).filter((u) => u === UP.UnitUpgrade.force)
          .length > 0,
      immuneBlast:
        (profile.keywords?.immune || []).filter(
          (i) => i === UP.UnitImmune.blast
        ).length > 0,
      immuneMeleePierce:
        (profile.keywords?.immune || []).filter(
          (i) => i === UP.UnitImmune.meleePierce
        ).length > 0,
      immunePierce:
        (profile.keywords?.immune || []).filter(
          (i) => i === UP.UnitImmune.pierce
        ).length > 0,
      ...
      tokens: {
        ...
        shield: profile.keywords?.shielded
          ? tokens.shield + profile.keywords.shielded
          : tokens.shield,
...
export function createDefenseInputsFromProfile(
  profile: UP.UnitProfile,
  upgrades: Array<UC.Upgrade>,
  tokens: T.DefenseTokens
): T.AttackInput {
