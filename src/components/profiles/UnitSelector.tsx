...
function UnitSelector(props: UnitSelectorProps): React.JSX.Element {
  const [matchingNames, setMatchingNames] = useState(
    getMatchingNameCount(props.selectedUnit.name)
  );
  ...
  const onSubtitleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const possibleUnits = props.units.filter(
      (u) =>
        u.name === props.selectedUnit.name &&
        doesSubtitleMatch(u.subtitle, newSubtitle)
    );
    if (possibleUnits.length > 0) {
      newUnit = possibleUnits[0];
    }
  ...
  return (
    <div>
      <div className="d-flex mx-auto">
        ...
        >
          {[...new Set(props.units.map((u) => u.name))].map((name) => (
      ...
      {matchingNames === 1 &&
        props.selectedUnit.subtitle &&
        props.selectedUnit.subtitle.length > 0 && (
          <div
            key={props.id + "-subtitle-one"}
      ...
      {matchingNames > 1 && (
        <div className="d-flex mx-auto mt-2">
          ...
          >
            {props.units
              .filter((u) => u.name === props.selectedUnit.name)
              .map((u) => (
                <option key={u.name + "-" + u.subtitle} value={u.subtitle}>
                  {u.subtitle}
                </option>
              ))}
          </select>
