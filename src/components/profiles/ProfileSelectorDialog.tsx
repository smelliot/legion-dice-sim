...
function ProfileSelectorDialog(
  props: ProfileSelectorDialogProps
): React.JSX.Element {
  ...
  function createDefaultState(): ProfileSelectorDialogState {
    ...
    return {
      ...
      unit: unit,
      weapons: [],
      weaponSelectionCount: Math.min(
        getMaxCompatibleWeapons(unit),
        unit.keywords?.arsenal === undefined ? 1 : unit.keywords.arsenal
      ),
      upgrades: [],
    };
  }
  ...
  function getNewStateObject(
    ...
    const newFaction =
      faction !== null
        ...
        : state?.faction
          ...
          : UP.Faction.rebel;
    const newRank =
      rank !== null ? rank : state?.rank ? state.rank : UP.Rank.commander;
    const newUnit =
      unit !== null && unit.faction === newFaction && unit.rank === newRank
        ? unit
        : unit === null &&
            newFaction == state?.faction &&
            newRank == state?.rank &&
            state?.unit
          ? state.unit
          : getFirstUnit(newFaction, newRank);
    const newWeapons =
      weapons !== null ? weapons : newUnit === state?.unit ? state.weapons : [];
    const newUpgrades =
      upgrades !== null
        ? upgrades
        : newUnit === state?.unit
    ...
    return {
      ...
      weapons: newWeapons,
      weaponSelectionCount: Math.min(
        getMaxCompatibleWeapons(newUnit),
        newUnit.keywords?.arsenal === undefined ? 1 : newUnit.keywords.arsenal
      ),
  ...
  function getFilteredUnits(
    faction: UP.Faction,
    rank: UP.Rank
  ): Array<UP.UnitProfile> {
    return UP.getUnits().filter(
      (profile) => profile.faction == faction && profile.rank == rank
    );
  }

  function getFilteredUpgradeTypes(): Array<UP.UnitUpgrade> {
    const filtered = state.unit.upgrades?.filter(
      (u) =>
        UC.getUpgrades().filter(
          (ufilter) => ufilter.type === u && isAvailable(ufilter)
        ).length > 0
    );
  ...
  function isAvailable(upgrade: UC.Upgrade, index?: number): boolean {
    ...
    upgrade.restrictions.forEach((r) => {
      ...
      if (
        r.upgrade &&
        !(state?.unit.upgrades && state.unit.upgrades.includes(r.upgrade))
      ) {
        match = false;
      }
  ...
  function renderWeapons(
    ...
    return children.length === 0 ? null : <div>{children}</div>;
  }
      ...
      <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title" id="{props.id + ModalLabel}">
              {props.upgradeAllowListName === AL.AllowListName.attack
                ? "Attack"
                : "Defense"}{" "}
              profile
            </h5>
              ...
              {getFilteredUpgradeTypes().map((utype, i) => (
                <div key={i} className="row justify-content-center my-2">
                  ...
                  <ItemSelector<UC.Upgrade>
                    ...
                    ariaLabel={"Upgrade " + i}
                    includeBlankItem={true}
                    items={UC.getUpgrades().filter(
                      (ufilter) =>
                        ufilter.type === utype && isAvailable(ufilter, i)
                    )}
