...
export class AppStateManager {
  ...
  public get attackEventHandlers(): AppStateAttackEventHandlers {
    return this._attackEventHandlers;
  }

  public get defenseEventHandlers(): AppStateDefenseEventHandlers {
    return this._defenseEventHandlers;
  }

  public get combatEventHandlers(): AppStateCombatEventHandlers {
    return this._combatEventHandlers;
  }

  public get behaviorEventHandlers(): AppStateBehaviorEventHandlers {
    return this._behaviorEventHandlers;
  }
  ...
  public updateOutputs(outputs: T.CombinedAttackOutput): void {
    ...
    newState.resultsVisibility =
      this.state.diceRolls === 1 ? T.ResultOutput.Single : T.ResultOutput.Graph;
    this.setState(newState);
  }

  public applyAttackStateProfile(
    profile: UP.UnitProfile,
    weapons: Array<UP.Weapon>,
    upgrades: Array<UC.Upgrade>
  ): void {
    const newState = this.cloneState();
    const input = AIA.createAttackInputsFromProfile(
      profile,
      weapons,
      upgrades,
      newState.inputs.offense.tokens
    );
  ...
  public applyDefenseStateProfile(
    profile: UP.UnitProfile,
    upgrades: Array<UC.Upgrade>
  ): void {
    const newState = this.cloneState();
    const input = DIA.createDefenseInputsFromProfile(
      profile,
      upgrades,
      newState.inputs.defense.tokens
    );
      ...
      outputs: {
        ...
        summary: {
          ...
          defenseSurge: this.cloneArray(
            this.state.outputs.summary.defenseSurge
          ),
  ...
  private createAttackEventHandlers(): AppStateAttackEventHandlers {
    return {
      ...
      handleSurgeConversionChange: (value: T.AttackSurgeConversion) =>
        this.handleAttackSurgeConversionChange(value),
      handleBlastChange: (active: boolean) => this.handleBlastChange(active),
      handleCriticalXChange: (active: boolean) =>
        this.handleCriticalXChange(active),
      handleCriticalXValueChange: (value: number) =>
        this.handleCriticalXValueChange(value),
      handleDuelistChange: (active: boolean) =>
        this.handleDuelistAttackChange(active),
      handleHighVelocityChange: (active: boolean) =>
        this.handleHighVelocityChange(active),
      handleImpactXChange: (active: boolean) =>
        this.handleImpactXChange(active),
      handleImpactXValueChange: (value: number) =>
        this.handleImpactXValueChange(value),
      handleIonXChange: (active: boolean) => this.handleIonXChange(active),
      handleIonXValueChange: (value: number) =>
        this.handleIonXValueChange(value),
      handleJediHunterChange: (active: boolean) =>
        this.handleJediHunterChange(active),
      handleLethalXChange: (active: boolean) =>
        this.handleLethalXChange(active),
      handleLethalXValueChange: (value: number) =>
        this.handleLethalXValueChange(value),
      handleMakashiMasteryChange: (active: boolean) =>
        this.handleMakashiMasteryChange(active),
      handleMarksmanChange: (active: boolean) =>
        this.handleMarksmanChange(active),
      handlePierceXChange: (active: boolean) =>
        this.handlePierceXChange(active),
      handlePierceXValueChange: (value: number) =>
        this.handlePierceXValueChange(value),
      handlePreciseXChange: (active: boolean) =>
        this.handlePreciseXChange(active),
      handlePreciseXValueChange: (value: number) =>
        this.handlePreciseXValueChange(value),
      handleRamXChange: (active: boolean) => this.handleRamXChange(active),
      handleRamXValueChange: (value: number) =>
        this.handleRamXValueChange(value),
      handleSharpshooterXChange: (active: boolean) =>
        this.handleSharpshooterXChange(active),
      handleSharpshooterXValueChange: (value: number) =>
        this.handleSharpshooterXValueChange(value),
    };
  }
  ...
  private createDefenseEventHandlers(): AppStateDefenseEventHandlers {
    return {
      ...
      incrementObservationTokenCount: () =>
        this.incrementObservationTokenCount(),
      incrementShieldTokenCount: () => this.incrementShieldTokenCount(),
      incrementSuppressionTokenCount: () =>
      ...
      handleArmorXValueChange: (value: number) =>
        this.handleArmorXValueChange(value),
      handleBlockChange: (active: boolean) => this.handleBlockChange(active),
      handleDangerSenseXChange: (active: boolean) =>
        this.handleDangerSenseXChange(active),
      handleDangerSenseXValueChange: (value: number) =>
        this.handleDangerSenseXValueChange(value),
      handleDeflectChange: (active: boolean) =>
        this.handleDeflectChange(active),
      handleDjemSoMasteryChange: (active: boolean) =>
        this.handleDjemSoMasteryChange(active),
      handleDuelistChange: (active: boolean) =>
        this.handleDuelistDefenseChange(active),
      handleHasForceUpgradesChange: (active: boolean) =>
        this.handleHasForceUpgradesChange(active),
      handleImmuneBlastChange: (active: boolean) =>
        this.handleImmuneBlastChange(active),
      handleImmuneMeleePierceChange: (active: boolean) =>
        this.handleImmuneMeleePierceChange(active),
      handleImmunePierceChange: (active: boolean) =>
        this.handleImmunePierceChange(active),
      handleImperviousChange: (active: boolean) =>
        this.handleImperviousChange(active),
      handleLowProfileChange: (active: boolean) =>
        this.handleLowProfileChange(active),
      handleOutmaneuverChange: (active: boolean) =>
        this.handleOutmaneuverChange(active),
      handleSoresuMasteryChange: (active: boolean) =>
        this.handleSoresuMasteryChange(active),
      handleUncannyLuckXChange: (active: boolean) =>
        this.handleUncannyLuckXChange(active),
      handleUncannyLuckXValueChange: (value: number) =>
        this.handleUncannyLuckXValueChange(value),
    };
  }
  ...
  private createCombatEventHandlers(): AppStateCombatEventHandlers {
    return {
      ...
      handleGuardianXValueChange: (value: number) =>
        this.handleGuardianXValueChange(value),
      handleProtectorChange: (active: boolean) =>
        this.handleProtectorChange(active),
  ...
  private handleProtectorChange(active: boolean) {
    const newState = this.cloneState();
    newState.inputs.combat.protector = active;
    this.setState(newState);
  }
  ...
  private createBehaviorEventHandlers(): AppStateBehaviorEventHandlers {
    return {
      handleDiceCountChange: (count: number) =>
        this.handleDiceCountChange(count),
      handleShowExpectedRangeChanged: (show: boolean) =>
        this.handleShowExpectedRangeChanged(show),
      handleShowSimplifiedViewChange: (show: boolean) =>
        this.handleShowSimplifiedViewChange(show),
    };
  }
  ...
  private loadSettings(): AppSettings {
    ...
    if (settingsString) {
      try {
        ...
        if (
          Object.prototype.hasOwnProperty.call(rawObject, "showExpectedRange")
        ) {
          settingsObject.showExpectedRange = Boolean(
            rawObject["showExpectedRange"]
          ).valueOf();
        }
        if (Object.prototype.hasOwnProperty.call(rawObject, "showSimpleView")) {
          settingsObject.showSimpleView = Boolean(
            rawObject["showSimpleView"]
          ).valueOf();
        }
      } catch {
        // ignore parse errors
