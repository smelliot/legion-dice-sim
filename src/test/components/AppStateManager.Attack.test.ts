...
describe("AppStateManager", () => {
  ...
  describe("attack", () => {
    ...
    it("handles changing surge conversion", () => {
      asm.attackEventHandlers.handleSurgeConversionChange(
        T.AttackSurgeConversion.Critical
      );
      expect(state?.inputs.offense.surge).toEqual(
        T.AttackSurgeConversion.Critical
      );
      asm.attackEventHandlers.handleSurgeConversionChange(
        T.AttackSurgeConversion.Hit
      );
      expect(state?.inputs.offense.surge).toEqual(T.AttackSurgeConversion.Hit);
      asm.attackEventHandlers.handleSurgeConversionChange(
        T.AttackSurgeConversion.Blank
      );
      expect(state?.inputs.offense.surge).toEqual(
        T.AttackSurgeConversion.Blank
      );
