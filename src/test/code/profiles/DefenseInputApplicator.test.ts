...
describe("DefenseInputApplicator", () => {
  ...
  it("applies armor X upgrade", () => {
    const profile = UP.getUnits().filter(
      (u) => u.name === "Wookiee Warriors" && u.faction === UP.Faction.rebel
    )[0];
    const upgrades = UC.getUpgrades().filter(
      (u) => u.name === "Battle Shield Wookiee (Defensive)"
    );
    ...
    expect(input.defense).toMatchSnapshot(
      "Wookie Warriors + Battle Shield Wookiee (Defensive)"
    );
  });
  ...
  it("applies cover upgrade (none to light)", () => {
    const profile = UP.getUnits().filter(
      (u) => u.name === "A-A5 Speeder Truck"
    )[0];
    const upgrades = UC.getUpgrades().filter(
      (u) => u.name === "Outer Rim Speeder Jockey"
    );
    ...
    expect(input.defense).toMatchSnapshot(
      "A-A5 Speeder Truck + Outer Rim Speeder Jockey"
    );
  });
  ...
  it("applies cover upgrade (light to heavy)", () => {
    ...
    expect(input.defense).toMatchSnapshot(
      "T-47 Airspeeder + Outer Rim Speeder Jockey"
    );
  });
  ...
  it("handles cover upgrade (heavy + upgrade)", () => {
    ...
    expect(input.defense).toMatchSnapshot(
      "cover upgrade (heavy +  Outer Rim Speeder Jockey)"
    );
  });
  ...
  it("applies cover upgrade (heavy upgrade)", () => {
    ...
    expect(input.defense).toMatchSnapshot(
      "T-47 Airspeeder + heavy cover upgrade"
    );
  });
  ...
  it("applies djem so mastery upgrade", () => {
    ...
    expect(input.defense).toMatchSnapshot(
      "General Grievous + djem so mastery upgrade"
    );
  });

  it("applies duelist upgrade", () => {
    const profile = UP.getUnits().filter(
      (u) => u.name === "Mandalorian Resistance"
    )[0];
    const upgrades = UC.getUpgrades().filter(
      (u) => u.name === "Beskad Duelist"
    );
    ...
    expect(input.defense).toMatchSnapshot(
      "Mandalorian Resistance + Beskad Duelist"
    );
  });

  it("applies immune: blast upgrade", () => {
    const profile = UP.getUnits().filter(
      (u) => u.name === "Phase I Clone Troopers"
    )[0];
    const upgrades = [
      {
    ...
    expect(input.defense).toMatchSnapshot(
      "Phase I Clone Troopers + immune: blast upgrade"
    );
  });
  ...
  it("applies outmaneuver upgrade", () => {
    const profile = UP.getUnits().filter(
      (u) => u.name === "Phase II Clone Troopers"
    )[0];
    const upgrades = UC.getUpgrades().filter(
      (u) => u.name === "Situational Awareness"
    );
    ...
    expect(input.defense).toMatchSnapshot(
      "Phase II Clone Troopers + Situational Awareness"
    );
  });
  ...
  it("applies shielded upgrade", () => {
    ...
    const upgrades = UC.getUpgrades().filter(
      (u) => u.name === "Personal Combat Shield"
    );
    ...
    expect(input.defense).toMatchSnapshot(
      "Sabine Wren + Personal Combat Shield"
    );
  });
  ...
  it("applies soresu mastery upgrade", () => {
    ...
    expect(input.defense).toMatchSnapshot(
      "General Grievous + soresu mastery upgrade"
    );
  });
