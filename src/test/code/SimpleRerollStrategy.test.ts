...
describe("SimpleRerollStrategy", () => {
  it("is retrieved from factory correctly", () => {
    ...
    // no armor or armor x
    expect(
      factory.selectStrategy(input, RerollReason.AimToken) instanceof
  ...
  it("shouldHandle returns true", () => {
    ...
    expect(strategy.shouldHandle(input, RerollReason.ObservationToken)).toEqual(
      true
    );
  });
  ...
  it("rerolls misses", () => {
    ...
    const indexes = strategy.pickRerollDice(
      input,
      rolls,
      rerollDiceCount,
      reason,
  ...
  it("rerolls multiple colors", () => {
    const input = T.createDefaultAttackInput();
    input.offense.preciseX.active = true;
    const strategy = new SimpleRerollStrategy();
